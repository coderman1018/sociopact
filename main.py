    ...
    if current.tocomplete != '':
        ...
        for x in mylist:
            if x != '':
                ...
                d[x] = [
                    post.author,
                    User.query.filter_by(username=post.author).first().email,
                    post.id
                ]
...
def tasks():
    ...
    return render_template("blog.html",
                           all_posts=posts[::-1],
                           now=current.username,
                           now2=current.address)
...
def edit_post(post_id):
    ...
    edit_form = CreatePostForm(title=post.title, body=post.body)
